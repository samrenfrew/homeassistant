...
// src/custom-thermostat.ts
var DEFAULT_SHOW_TITLE = true;
var CARD_TYPE = "custom:custom-thermostat";
var INNER_CARD_STYLES = `
  ha-card {
    width: 100% !important;
    margin: 0 auto;
    background: none;
...
var CustomThermostatCard = class extends i4 {
  static async getConfigElement() {
    return document.createElement("custom-thermostat-editor");
  }
  static getStubConfig(hass, entities) {
    const entity = entities.find(
      (candidate) => ["climate.", "water_heater."].some(
  ...
  setConfig(config) {
    ...
    const [domain] = config.entity.split(".");
    if (!["climate", "water_heater"].includes(domain)) {
      throw new Error(
        "Entity must be from the climate or water_heater domain"
      );
    }
    this._config = {
      ...config,
      show_title: config.show_title ?? DEFAULT_SHOW_TITLE
    };
    void this._createOrUpdateInnerCard();
  }
  getCardSize() {
    return this._innerCard?.getCardSize?.() ?? 7;
  }
  getGridOptions() {
  ...
  render() {
    ...
    return b2`${this._innerCard}`;
  }
  updated() {
    if (this.hass && this._innerCard) {
      this._innerCard.hass = this.hass;
    }
    this._queuePatch();
  }
  disconnectedCallback() {
    ...
    if (this._patchTimer) {
      window.clearTimeout(this._patchTimer);
      this._patchTimer = void 0;
    }
    if (this._patchFrame) {
      window.cancelAnimationFrame(this._patchFrame);
      this._patchFrame = void 0;
    }
  }
  async _createOrUpdateInnerCard() {
    if (!this._config) {
      return;
    }
    const cardConfig = this._buildInnerConfig(this._config);
    if (!this._innerCard) {
      const helperLoader = window.loadCardHelpers;
      if (!helperLoader) {
        throw new Error("Home Assistant card helpers are unavailable");
      }
      const helpers = await helperLoader();
      this._innerCard = await Promise.resolve(
        helpers.createCardElement(cardConfig)
      );
      this._innerCard.addEventListener("ll-rebuild", (event) => {
        event.stopPropagation();
        this._innerCard = void 0;
        void this._createOrUpdateInnerCard();
      });
    } else {
  ...
  _buildInnerConfig(config) {
    const { show_title, ...passthroughConfig } = config;
    const innerConfig = {
      ...passthroughConfig,
      type: "thermostat"
    };
  ...
  _queuePatch() {
    ...
    this._patchTimer = window.setTimeout(() => this._applyVisualPatches(), 0);
    if (this._patchFrame) {
      window.cancelAnimationFrame(this._patchFrame);
    }
    this._patchFrame = window.requestAnimationFrame(
      () => this._applyVisualPatches()
    );
  }
  _injectStyles(root, css, id) {
  ...
  _applyVisualPatches() {
    if (!this._innerCard?.shadowRoot) {
      return;
    }
    const innerRoot = this._innerCard.shadowRoot;
    this._injectStyles(innerRoot, INNER_CARD_STYLES, "custom-thermostat-styles");
    const showTitle = this._config?.show_title ?? DEFAULT_SHOW_TITLE;
    const titleElement = innerRoot.querySelector(".title");
    if (titleElement) {
      titleElement.style.display = showTitle ? "" : "none";
    }
    const controls = innerRoot.querySelectorAll(
      "ha-state-control-climate-temperature, ha-state-control-water_heater-temperature"
    );
    controls.forEach((control) => {
      ...
      this._injectStyles(
        controlRoot,
        STATE_CONTROL_STYLES,
        "custom-thermostat-control"
      );
      const bigNumbers = controlRoot.querySelectorAll(
        "ha-big-number"
      );
      bigNumbers.forEach((bigNumber) => {
        const bigNumberRoot = bigNumber.shadowRoot;
        if (!bigNumberRoot) {
          return;
        }
        const decimal = bigNumberRoot.querySelector(".decimal");
        if (!decimal) {
          return;
        }
        ...
        const hasFraction = Number.isFinite(numericValue) && Math.abs(numericValue - Math.trunc(numericValue)) > 1e-6;
        if (hasFraction) {
          decimal.style.opacity = "";
          decimal.style.visibility = "";
        } else {
...
CustomThermostatCard.properties = {
  hass: { attribute: false },
  _config: { state: true }
};
CustomThermostatCard.styles = i`
    :host {
      display: block;
      height: 100%;
    }
  `;
var CustomThermostatCardEditor = class extends i4 {
  constructor() {
    super(...arguments);
    this._computeLabel = (schema) => {
      switch (schema.name) {
        ...
        case "show_current_as_primary":
          return "Show current temperature as primary";
        default:
          return schema.name;
      }
    };
...
CustomThermostatCardEditor.properties = {
  hass: { attribute: false },
  _config: { state: true }
};
CustomThermostatCardEditor.styles = i`
    :host {
      display: block;
      padding: 8px 0 0;
    }
  `;
if (!customElements.get("custom-thermostat")) {
  customElements.define("custom-thermostat", CustomThermostatCard);
}
if (!customElements.get("custom-thermostat-editor")) {
  customElements.define(
    "custom-thermostat-editor",
    CustomThermostatCardEditor
  );
}
var cards = window.customCards || [];
if (!cards.some((card) => card.type === CARD_TYPE)) {
  cards.push({
    type: CARD_TYPE,
    name: "Custom Thermostat Card",
    description: "Wraps the built-in thermostat card with optional title and clean integer center values."
  });
}
